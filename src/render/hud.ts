...
const FONT = { fontFamily: 'monospace', fontSize: '8px', color: hex(PAL.uiText) };
...
export class Hud {
  ...
  constructor(private scene: Phaser.Scene, private worldY: number) {
    ...
    this.name = scene.add.text(14, 4, '', { ...FONT, fontStyle: 'bold' }).setDepth(101).setScrollFactor(0);
    this.aim = scene.add.text(150, 4, '', FONT).setDepth(101).setScrollFactor(0);
    this.weapon = scene.add.text(300, 4, '', FONT).setDepth(101).setScrollFactor(0);
    this.windText = scene.add.text(NATIVE_W - 150, 4, 'WIND', { ...FONT, color: hex(PAL.uiTextDim) }).setDepth(101).setScrollFactor(0);
    this.status = scene.add.text(6, NATIVE_H - 10, '', { ...FONT, fontSize: '7px', color: hex(PAL.uiTextDim), backgroundColor: hex(PAL.uiInk) }).setDepth(101).setScrollFactor(0);
    this.banner = scene.add
      .text(NATIVE_W / 2, 120, '', { fontFamily: 'monospace', fontSize: '18px', color: hex(PAL.uiText), stroke: hex(PAL.uiInk), strokeThickness: 4, align: 'center' })
      .setOrigin(0.5)
      .setDepth(120)
      .setScrollFactor(0)
      .setAlpha(0);
  ...
  update(world: World, current: Tank | null, statusLine: string): void {
    ...
    if (current) {
      ...
      g.fillStyle(team.mid, 1).fillRect(4, 4, 6, 14);
      g.fillStyle(team.lit, 1).fillRect(4, 4, 6, 2);
      ...
      // Power bar
      g.fillStyle(PAL.uiPanel, 1).fillRect(150, 14, 120, 5);
      g.fillStyle(PAL.uiEdge, 1).fillRect(151, 15, Math.round(118 * (current.power / 100)), 3);
    ...
    const wx = NATIVE_W - 110;
    const ww = 96;
    g.fillStyle(PAL.uiPanel, 1).fillRect(wx, 6, ww, 8);
    g.fillStyle(PAL.uiPanelLit, 1).fillRect(wx + ww / 2, 5, 1, 10);
    ...
    if (frac >= 0) g.fillRect(wx + ww / 2 + 1, 8, len, 4);
    else g.fillRect(wx + ww / 2 - len, 8, len, 4);
    ...
    for (const t of world.tanks) {
      ...
      if (!e) {
        e = {
          ...
          tag: this.scene.add.text(0, 0, t.name, { ...FONT, fontSize: '7px' }).setOrigin(0.5, 1).setDepth(61),
        };
      ...
      const y = Math.round(t.y + this.worldY - t.halfHeight * 2 - 22);
